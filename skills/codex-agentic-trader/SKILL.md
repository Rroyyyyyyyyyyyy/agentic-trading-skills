...
- 定时唤醒可在 policy 规定的最多 5 分钟内迟到，但不得提前触发。一律以 `scheduled_for_et` / `cadence_slot_id` 作为逻辑时段，同一时段的重复唤醒只对账，不重复研究、下单或发日报；
...
每次完整操作判断都必须先复查上一轮观察池，再扫描新标的，不能每轮从零开始。若本轮完成当前层级扫描后仍没有合格候选，下一次完整操作判断按 policy 的 `candidate_research.progressive_screen_counts` 扩大广筛范围（10→20→40→60→100），同时保留并复查最多 12 只上一轮强候选。达到 100 只后仍无合格候选时，每轮至少轮换 20 只此前未覆盖或已久未复核的合格大型流动股；不能把“没交易”当作重置候选池的理由。

广筛只做硬门与相对强度过滤，深度研究最多 12 只，优先包含仍接近触发的旧候选。候选只有在硬门失效、趋势/相对强度失效、论点破坏或证据过期时才能移出观察池；进入财报回避窗口只标记为暂不可新建仓，不得假装它从未被研究。每轮把扫描层级、复查名单、新增覆盖、淘汰原因和下一层级写入 `research` 事件，供下一次完整判断继承。

扩大的是研究覆盖，不是风险或交易频率。没有合格候选仍保持现状；不得因为扩大扫描就降低准入、增加持仓数、突破换手/订单/仓位上限或把“无候选”本身当作下单触发。
...
收盘后追加一个 `daily_review` 事件，保存事实、决策、成交结果、错失机会、反证、当日双基准胜负、滚动基准差、挑战进度、风险姿态、候选扫描层级、持续观察池、新增/淘汰名单及原因、最强教训和次日条件。下一次盘前分析先读取最近 20 个 `daily_review` 与成交行为统计，让新分析继承历史证据。每天最多提出一个有测试、评价指标和回滚方案的改进；自动积累知识不等于自动放宽 policy、账户边界或风险上限。
