...
- 没有合格候选时把广筛从 10 递进到 20、40、60、100，并持续复查最多 12 只上一轮强候选；达到上限后轮换新标的，不因未交易而清空研究上下文；
...
- 以完整本金可用后的下一交易日起连续 100 个日历日实现 2,500 美元净利润为一级目标，锁定基线并剔除后续外部现金流；
- 以每个完成交易日的策略净收益同时跑赢 VOO 与 QQQ 为每日评分目标，并保留滚动双基准考核；
- 进度显著落后且趋势、回撤、波动和数据条件全部健康时，可在原有硬上限内切换受控进攻配置；
...
盘中频率由 `cadence_gate.py` 确定：购买力至少覆盖最小订单和现金缓冲时每 30 分钟判断；不足时只在 10:00、12:00、14:00、16:00 ET 做完整判断，重大风险和订单异常除外。`performance_review.py` 客观计算100天/$2,500净利润进度、每日/滚动 VOO/QQQ 胜负及受控进攻资格，但不直接授权交易，也不承诺收益。
