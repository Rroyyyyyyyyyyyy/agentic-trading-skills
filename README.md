...
盘中频率由 `cadence_gate.py` 确定：购买力至少覆盖最小订单和现金缓冲时每 30 分钟判断；不足时只在 10:00、12:00、14:00、16:00 ET 做完整判断，重大风险和订单异常除外。`performance_review.py` 客观计算100天/$2,500净利润进度并比较 VOO/QQQ，但不直接授权交易，也不承诺收益。
